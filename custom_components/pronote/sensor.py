...
class PronoteInformationAndSurveysSensor(PronoteGenericSensor):
    ...
    def extra_state_attributes(self):
        """Return the state attributes."""
        attributes = super().extra_state_attributes
        # Already formatted by the coordinator, which holds a live client.
        informations = self.coordinator.data["information_and_surveys"]

        attributes["unread_count"] = (
            None
            if informations is None
            else sum(1 for information in informations if information["read"] is False)
        )
        attributes["information_and_surveys"] = (
...
class PronoteDiscussionsSensor(PronoteGenericSensor):
    ...
    @property
    def native_value(self):
        """Return the number of discussions.

        Computed on every read, so the count follows the coordinator refreshes
        instead of being frozen at entity creation time.
        """
